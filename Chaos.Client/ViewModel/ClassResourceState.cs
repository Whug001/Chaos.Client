...
///     (<c>SetIchorState</c>, opcode 115), the Berserker's Rage (<c>SetRageState</c>, opcode 116), the Warlord's
///     Valor (<c>SetValorState</c>, opcode 117) or the Assassin's Malice (<c>SetMaliceState</c>, opcode 118).
///     Each is a single 0-100 byte and they all drive the same strip, so they share one state object: whichever
///     resource the server last reported is the one on display, and no character ever carries more than one.
///     Never computed here - the server is authoritative and this holds whatever it last sent.
/// </summary>
public sealed class ClassResourceState
{
    ...
    public void SetValor(byte valor) => Set(ClassResourceKind.Valor, valor);
